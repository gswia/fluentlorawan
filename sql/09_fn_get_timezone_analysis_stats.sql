CREATE OR REPLACE FUNCTION get_timezone_analysis_stats(
...
BEGIN
    ...
    v_window_start   := v_window_end   - INTERVAL '24 hours';
    v_previous_end   := v_window_start;
    v_previous_start := v_previous_end - INTERVAL '24 hours';

    RETURN QUERY
    WITH temp_current AS (
        SELECT * FROM get_temperature_window_stats(p_timezone, v_window_start, v_window_end)
    ),
    temp_previous AS (
        SELECT * FROM get_temperature_window_stats(p_timezone, v_previous_start, v_previous_end)
    ),
    hum_current AS (
        SELECT * FROM get_humidity_window_stats(p_timezone, v_window_start, v_window_end)
    ),
    hum_previous AS (
        SELECT * FROM get_humidity_window_stats(p_timezone, v_previous_start, v_previous_end)
    ),
    illum_current AS (
        SELECT * FROM get_illumination_window_stats(p_timezone, v_window_start, v_window_end)
    ),
    illum_previous AS (
        SELECT * FROM get_illumination_window_stats(p_timezone, v_previous_start, v_previous_end)
    ),
    door_current AS (
        SELECT * FROM get_door_window_stats(p_timezone, v_window_start, v_window_end)
    ),
    door_previous AS (
        SELECT * FROM get_door_window_stats(p_timezone, v_previous_start, v_previous_end)
    ),
    vib_current AS (
        SELECT * FROM get_vibration_window_stats(p_timezone, v_window_start, v_window_end)
    ),
    vib_previous AS (
        SELECT * FROM get_vibration_window_stats(p_timezone, v_previous_start, v_previous_end)
    ),
    all_sensors AS (
        SELECT
            tc.site_id,
        ...
        -- NOTE: Door avg/median/stddev/p75/p90/iqr are over hourly open_duration_sum
        -- values (24 data points), not over individual readings like the other sensor types.
        SELECT
            dc.site_id,
            dc.device_id,
            dc.sensor_id,
